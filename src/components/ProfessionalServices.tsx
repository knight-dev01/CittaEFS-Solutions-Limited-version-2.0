  ...
  return (
    <section id="services" className="py-20 sm:py-28 bg-white text-slate-800 relative overflow-hidden border-b border-slate-200">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Human Consulting & Client Collaboration Visual Card with 3D Tilt */}
        <TiltCard3D maxTilt={5} scaleHover={1.01} className="mb-14 sm:mb-18 rounded-3xl">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5 }}
            viewport={{ once: true }}
            className="rounded-3xl overflow-hidden bg-slate-50 border border-slate-200 shadow-xl"
          >
        ...
        {/* Services Grid with 3D Tilt Flow */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 items-stretch">
          {services.map((srv, idx) => {
            const Icon = srv.icon;
            return (
              <TiltCard3D key={idx} maxTilt={8} scaleHover={1.025} className="h-full rounded-3xl">
                <motion.div
                  initial={{ opacity: 0, y: 15 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.4, delay: idx * 0.05 }}
                  viewport={{ once: true }}
                  className="p-6 sm:p-8 bg-white border border-slate-200 rounded-3xl hover:border-[#2582ff]/40 hover:shadow-xl transition-all duration-300 text-left flex flex-col justify-between group h-full shadow-xs"
                >
                  <div className="space-y-4">
                    <div className={`p-3 rounded-2xl w-12 h-12 flex items-center justify-center border transition-transform duration-300 group-hover:scale-110 ${srv.color}`}>
                  ...
                  <div className="pt-6 flex items-center text-[10px] font-mono text-slate-400 uppercase tracking-wider group-hover:text-[#2582ff] transition-colors">
                    ...
                    <ArrowUpRight className="w-3.5 h-3.5 ml-1 transform group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />
                  </div>
                </motion.div>
              </TiltCard3D>
            );
